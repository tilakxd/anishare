...
function ListCards({ url, title, clicked, _id, deleteAnime }: { url: string; title: string, clicked: boolean, _id: string, deleteAnime:(_id: string)=> any; }) {
  const [complete, setComplete] = useState(false);

  const completeAdd = async (_id: string) => {
    try {
      const response = await axios.put('http://localhost:3000/api/complete', { _id });
  ...
  useEffect(() => {
    try {
      const fetchCompletion =  async (_id: string) => {
        const response = await axios.post('http://localhost:3000/api/complete', { _id });
      console.log(response.data.completed)
  ...
  return (
    <>
      ...
      <div className="w-full max-w-72 border-2 rounded-lg items-center flex flex-col p-2">
        <button onClick = {handleClick} className="flex place-items-end"><svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" viewBox="0 0 24 24"><path fill="#ffffff" d="M19 4h-3.5l-1-1h-5l-1 1H5v2h14M6 19a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V7H6z"/></svg></button>
